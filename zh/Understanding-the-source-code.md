...
对于不使用IDE但是想要对代码进行调试的情况，可以通过将某一部的变量进行输出的方式来查看变量的值，或者变量的类型，分析每一步具体输出的变量的值和该值可能进行的操作，这种方式的优点是执行速度快，从头到尾快速追踪一个或几个变量变量，比较适合于不能添加断点，单步调试比较困难的情况，同时，与添加断点单步调试相比，输出变量的方法可以使不同时期、不同处理过程的变量同时出现在Console中，可以进行对比查看某一变量经过某一过程之后发生的变化。

在Python语言中，我经常对数据进行处理的过程中，我经常会使用输出变量和断点调试结合的方式进行，因为断点调试中变量的值是实时变化的，想要对比就需要进行输出来对比变化。
